...
con.connect(function(err) {
  ...
  con.query( quer1 , function (err, result, fields) {
    ...
    {"label":"Potencia 1","type":"number"},
    {"label":"Potencia 2","type":"number"},
    {"label":"Potencia 3","type":"number"},
    {"label":"Potencia 4 ","type":"number"}]
    var rows

    rows=[]
    for (var i = 0; i < result.length; i++){
      var rowi= {"c":[{"v":"Date(" +result[i].datetime+")"},
      {"v":result[i].P1},{"v":result[i].P2},
      {"v":result[i].P3},{"v":result[i].P4}]}
      rows.push(rowi)
